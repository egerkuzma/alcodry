...
Трекер этапов воздержания от алкоголя. Одна лестница этапов, срывы, штрафные
недели, наградные окна. На одного пользователя, self-hosted, без аккаунтов.

Приложение только считает: оценок, советов и мотивационных текстов в интерфейсе
нет и не будет.
...
**Этап** — период, который нужно продержаться без алкоголя. Лестница из пяти
этапов: 1, 2, 3, 3 и 4 недели. Чистое прохождение — ровно 13 недель, три месяца.

**Срыв** — любое количество алкоголя в дни этапа, вплоть до одной рюмки,
исключений нет. Срыв добавляет штрафную неделю к текущему этапу и обнуляет
отсчёт, но **не двигает номер этапа**: цикл завершается только по числу честно
закрытых этапов, никогда по календарному времени.
...
**Окно** — три дня после закрытия этапа, когда алкоголь разрешён и срывом не
считается. Кнопка «Выпил» в окне пишется в журнал для статистики и ничего не
ломает. Окно можно закрыть досрочно, а если его не закрыть — оно истечёт само.

**Досрочно закрыть этап нельзя.** Проверка на сервере, а не только в интерфейсе:
`POST /api/stage-done` до срока возвращает 409.
