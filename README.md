...
В [`mac/`](mac/) лежит приложение для строки меню — чтобы не держать вкладку
открытой. Счётчик виден всегда, действия в один клик:
...
[лестница] 2/14
  ...
  ─────────────────
  Открыть в браузере · Обновить · Адрес сервера… · Запускать при входе ✓
```

В окне вместо двух первых действий — «Выпил» и «Начать этап сейчас», а в строке
меню `окно 3д`.

Своей базы у клиента нет: он ходит в тот же `/api/status` и те же `POST`, что и
веб-морда, и показывает ровно то, что ответил сервер. Разъехаться им нечем. Это
и есть тот «слой поверх API», под который всё проектировалось.

```sh
./mac/build.sh          # → mac/build/alcodry.app и alcodry.dmg, 126 КБ
```

Один файл на Swift, ни Xcode, ни зависимостей — только Command Line Tools.
Автозапуск включается галочкой в меню. Установка, подпись и разрешения —
